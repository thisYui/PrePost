...
if command -v julia &> /dev/null && [ -f "Project.toml" ]; then
    ...
    julia --project=. -e "using ArgParse, CSV, DataFrames, BenchmarkTools, Plots, StatsBase, DataStructures; println(\"OK: required packages loaded\")" 2>/dev/null
